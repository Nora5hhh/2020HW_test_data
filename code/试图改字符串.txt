...
string result[threadCnt][5][2000000];
...
private:
	...
	void dfs(unsigned int begin, unsigned int index, unsigned int* path, string pathStr, unsigned int* visit, int count, int threadId);
};
...
void searchEdge::dfs(unsigned int begin, unsigned int index, unsigned int* path,string pathStr, unsigned int* visit, int count, int threadId)
{
	path[count] = index;
	pathStr += to_string(index-1) + ',';
	++count;
	visit[index] = 1;
	int i = 0;
	...
	for (; i < nLen && Gra[index][i] <= begin; ++i)
	{
		...
		{
			auto sLen = pathStr.size();
			pathStr[sLen - 1] = '\n';
			result[threadId][count - 3][cirArr[threadId][count - 3]] = pathStr;
	...
	if (count < 5)
	{
		for (; i < nLen; ++i) {
			...
			if (curr != begin && visit[curr] == 0) {
				dfs(begin, curr, path,pathStr, visit, count, threadId);
			}
		}
	}
	if (reachBegin2[threadId][index][1] == begin && count == 5)
	{
		...
		for (int j = 1; j <= elem[0]; ++j)
		{
			...
			path[5] = next;
			pathStr += to_string(index-1) + ',';
			pathStr += to_string(next-1) + '\n';
			result[threadId][3][cirArr[threadId][3]] = pathStr;
			++cirArr[threadId][3];
		}
	}
	if (reachBegin3[threadId][index][1] == begin && count == 5)
	{
		...
		for (int j = 1; j <= elem[0]; ++j)
		{
			...
			path[6] = next2;          //gt
			pathStr += to_string(index-1) + ',';
			pathStr += to_string(next1-1) + ',';
			pathStr += to_string(next2-1) + '\n';
			result[threadId][4][cirArr[threadId][4]] = pathStr;
			++cirArr[threadId][4];
		}
	}
	visit[index] = 0;
...
void searchEdge::findRing(int threadId, unsigned int beginIndex, unsigned int endIndex)
{
	for (auto i = beginIndex; i < endIndex; ++i)
	{
		...
		if (outDgr[begin] > 0 && inDgr[begin] > 0)
		{
			...
			unsigned int path[7];
			string pathStr;
			dfs(begin, begin, path, pathStr, visit[threadId], 0, threadId);
		}
	}
}
...
void searchEdge::storeResult() {
	...
	for (int i = 0; i < subIndex; ++i)
	{
		for (int t = 0; t < 4; ++t)
		{
			...
			{
				auto curStr = result[t][i][k];
				auto sLen = curStr.size();
				memcpy(buf, curStr.c_str(), sLen);
				buf += sLen;
			}
		}
	}
