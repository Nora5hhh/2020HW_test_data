...
unsigned int reachBegin2[node][2];
unsigned int idMap[250][250];
unsigned long int idMap3[25000][5000];
unsigned int idMap3_2[25000][5000];
unsigned int reachBegin1[node];
unsigned int reachBegin3[node][2];
unsigned int idPoint2[250];
class searchEdge {
public:
...
void searchEdge::storeGra3(unsigned int begin) {
	...
	for (int i = 0; i < inDgr[begin]; i++) {
		...
		reachBegin1[gi] == begin;
		for (int k = 0; k < inDgr[gi]; ++k)
		{
			unsigned int gk = revGra[gi][k];
			if (outDgr[gk] == 0 || inDgr[gk] == 0 || gk <= begin) continue;
			if (reachBegin2[gk][1] != begin)
			{
				reachBegin2[gk][0] = subLen;
				reachBegin2[gk][1] = begin;
				idMap[subLen++][0] = 0;
				idPoint2[pointLen++] = gk;
			}
			add = reachBegin2[gk][0];
			idMap[add][++idMap[add][0]] = gi;
		}

	}
	sort(idPoint2, idPoint2 + pointLen);
	for (int j = 0; j < pointLen; ++j)
	{
		unsigned int gj = idPoint2[j];            //倒数第二个指向begin的
		subLen = reachBegin2[gj][0];
		for (int t = 1; t <= idMap[subLen][0]; ++t)
		{
			unsigned int gt = idMap[subLen][t];       //倒数第一个指向begin
			for (int k = 0; k < inDgr[gj]; ++k)
			{
				unsigned int gk = revGra[gj][k];
				if (gk <= begin || gk == gt || inDgr[gk] == 0 || outDgr[gk] == 0)continue;
				if (reachBegin3[gk][1] != begin)
				{
					reachBegin3[gk][0] = subLen3;
					reachBegin3[gk][1] = begin;
					idMap3[subLen3++][0] = 0;
					idMap3_2[subLen3 - 1][0] = 0;
				}
				add = reachBegin3[gk][0];
				idMap3[add][++idMap3[add][0]] = gj;
				idMap3_2[add][++idMap3_2[add][0]] = gt;
			}
		}
	}
}
...
void searchEdge::dfs(unsigned int begin, unsigned int index, unsigned int* path, unsigned int* visit, int count)
{
	...
	if (reachBegin2[index][1] == begin && count == 5)
	{
		auto elem = idMap[reachBegin2[index][0]];
		for (int j = 1; j <= elem[0]; ++j)
		{
			auto next = elem[j];
			if (visit[next] == 1) continue;
			path[5] = next;
			memcpy(result[3][cirArr[3]], path, 6 * sizeof(unsigned int));
			++cirArr[3];
			++cicleCnt;
		}
	}
	if (reachBegin3[index][1] == begin && count == 5)
	{
		auto elem = idMap3[reachBegin3[index][0]];
		for (int j = 1; j <= elem[0]; ++j)
		{
			unsigned int next1 = elem[j];
			unsigned int next2 = idMap3_2[reachBegin3[index][0]][j];
