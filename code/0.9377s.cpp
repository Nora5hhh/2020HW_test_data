...
private:
	...
	void init();
	void dfs(int begin, int index, unsigned int* path, unsigned int* visit, int count);
};
...
void searchEdge::dfs(int begin, int index, unsigned int* path, unsigned int* visit, int count)
{
	path[count] = index;
	++count;
	visit[index] = 1;
