...
private:
	...
	bool loadTestData();
	void dfs(int begin, int index, vector<unsigned int>& path, int result_subindex);
...
void searchEdge::dfs(int begin, int index, vector<unsigned int>& path,int result_subindex)
{
	...
	if (path.size() < 6)
	{
		for (; it!=now.end(); ++it) {
			if (*it != begin) {
				dfs(begin, *it, path, result_subindex);
			}
		}
	}
	else if (path.size() == 6)
	{
		if (flag[begin][index] == true)
		{
			...
			for (auto next : elem)
			{
				...
				if (!checkRepeat(temp))
					continue;
				result_sub[result_subindex].push_back(temp);
			}
		}
	}
	path.pop_back();
