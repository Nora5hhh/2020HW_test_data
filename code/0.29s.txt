...
const unsigned int sizeTable[6] = { 9,99,999,9999,99999,999999 };
...
unsigned int intToString(unsigned int num, char* buf)
{
	unsigned int i = 0;
	for (;; ++i)
	{
...
void searchEdge::init() {
	...
	for (int i = 0; i < idCnt; i++) {
		subLen = intToString(pos[i] - 1, outStr[pos[i]] + 1);
		outStr[pos[i]][++subLen] = ',';
		outStr[pos[i]][0] = (char)subLen;
		subLen = intToString(pos[i] - 1, lstStr[pos[i]] + 1);
		lstStr[pos[i]][++subLen] = '\n';
