...
unsigned int idMoney[threadCount][500][500];
subNode  idMap[threadCount][500][500];
subNode  idMap3[threadCount][9000][500];
subNode  idMap3_2[threadCount][9000][500];
...
void bucketSort(unsigned int* a, unsigned int n, unsigned int max)
{
	unsigned int i, j;
	unordered_map<unsigned int, unsigned int> buckets;
	if (a == NULL || n < 1 || max < 1)
		return;
	for (i = 0; i < n; i++)
		buckets[a[i]]++;
...
void searchEdge::init() {
	...
	while (*p >= '0' && *p <= '9') {
		...
		nodData[cnt] = curNum;
		maxId = maxId > curNum ? maxId : curNum;
		++cnt;
		curNum = 0;
		p = q + 1;
		q = p;
...
void searchEdge::storeGra3(unsigned int begin, int threadId) {
	...
	for (int i = 0; i < inDgr[begin]; i++) {
		...
		for (int k = 0; k < inDgr[gi]; ++k)
		{
			...
			if (revGra[begin][i].money > (long long)revGra[gi][k].money * 3 || (long long)revGra[begin][i].money * 10 < revGra[gi][k].money * 2)continue;
	...
	for (uint32_t j = 0; j < outDgr[begin]; j++) {
		...
		if (reachBegin3[threadId][index3][1] == begin)
		{
			...
			for (int j = 1; j <= elem[0].sub; ++j)
			{
				...
				if ((long long)elem[j].money > money3 * 3 || (long long)elem[j].money * 10 < 2 * money3 || money3 > money1 * 3 || money3 * 10 < 2 * money1)continue;
				for (int i = 0; i < 2; i++) {
