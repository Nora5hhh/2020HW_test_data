...
subNode Gra[200000][100];
subNode revGra[200000][100];
...
subNode  idMap3[threadCount][9000][500][2];
...
void searchEdge::init() {
	...
	for (int i = 0; i < cnt; ++i)             //构建正向图与反向图
	{
		...
		Gra[u][outDgr[u]].sub = v;
		Gra[u][outDgr[u]].money = w;
		//elem.sub = u;
		//revGra[v].emplace_back(elem);
		revGra[v][inDgr[v]].sub = u;
		revGra[v][inDgr[v]].money = w;
		++outDgr[u];
		++inDgr[v];
	}
