...
unsigned int idMoney[4][1000][1000];
subNode  idMap[4][1000][1000];
subNode  idMap3[4][10000][1000];
subNode  idMap3_2[4][10000][1000];
unsigned int reachBegin1[4][node];
unsigned int reachBegin3[4][node][2];
unsigned int idPoint2[4][1000];
...
void searchEdge::storeGra3(unsigned int begin, int threadId) {
	...
	for (int i = 0; i < inDgr[begin]; i++) {
		...
		for (int k = 0; k < inDgr[gi]; ++k)
		{
			...
			idMap[threadId][ind][idMap[threadId][ind][0].sub].money = revGra[gi][k].money;//gk到gi的money
			idMoney[threadId][ind][++idMoney[threadId][ind][0]] = revGra[begin][i].money;
		}
	}
	...
	for (int j = 0; j < pointLen; ++j)
	{
		...
		for (int t = 1; t <= idMap[threadId][subLen][0].sub; ++t)
		{
			...
			for (int k = 0; k < inDgr[gj]; ++k)
			{
				...
				if (pre_money2 / revGra[gj][k].money > 3 || pre_money2 / revGra[gj][k].money < 0.2)continue;
				...
				idMap3[threadId][ind][idMap3[threadId][ind][0].sub].money = revGra[gj][k].money;  //gk->gj的money
				idMap3_2[threadId][ind][++idMap3_2[threadId][ind][0].sub].sub = gt;
				idMap3_2[threadId][ind][idMap3_2[threadId][ind][0].sub].money = pre_money;       //gt->begin的money
			}
		}
	}
}
...
void searchEdge::dfs_circle(unsigned int begin, unsigned int* path, unsigned int* visit, int threadId, int zonId) {
	...
	float pre_money1, pre_money2, pre_money3, pre_money4, pre_money5;
	uint32_t money1, money2, money3, money4, money5, money6, money7;
	path[0] = begin;
	visit[begin] = 1;
	//if(begin==5013) cout<<"hi";        进来l
	if (reachBegin3[threadId][begin][1] == begin)
	{
		...
		for (int j = 1; j <= elem[0].sub; ++j)
		{
			...
			money1 = idMap3_2[threadId][reachBegin3[threadId][begin][0]][j].money;
			money2 = elem[j].money;
			pre_money1 = (float)money2 / money1;
			if (pre_money1 > 3 || pre_money1 < 0.2)continue;
	...
	for (uint32_t j = 0; j < outDgr[begin]; j++) {
		...
		if (reachBegin3[threadId][index3][1] == begin)
		{
			...
			for (int j = 1; j <= elem[0].sub; ++j)
			{
				...
				if (visit[next1] == 1 || visit[next2] == 1) continue;
				money1 = idMap3_2[threadId][reachBegin3[threadId][index3][0]][j].money;
				pre_money1 = (float)elem[j].money / money3;
				pre_money2 = (float)money3 / money1;
				if (pre_money1 > 3 || pre_money1 < 0.2 || pre_money2>3 || pre_money2 < 0.2)continue;
				for (int i = 0; i < 2; i++) {
		...
		for (uint32_t k = 0; k < outDgr[index3]; k++) {
			...
			if (reachBegin3[threadId][index4][1] == begin)
			{
				...
				for (int j = 1; j <= elem[0].sub; ++j)
				{
					...
					if (visit[next1] == 1 || visit[next2] == 1) continue;
					money1 = idMap3_2[threadId][reachBegin3[threadId][index4][0]][j].money;
					money2 = elem[j].money;
					pre_money1 = (float)money3 / money1;
					pre_money2 = (float)money4 / money3;
					pre_money3 = (float)money2 / money4;
					if (pre_money1 > 3 || pre_money1 < 0.2 || pre_money2>3 || pre_money2 < 0.2 || pre_money3>3 || pre_money3 < 0.2)continue;
					for (int i = 0; i < 3; i++) {
			...
			for (uint32_t m = 0; m < outDgr[index4]; m++) {
				...
				if (reachBegin3[threadId][index5][1] == begin)
				{
					...
					for (int j = 1; j <= elem[0].sub; ++j)
					{
						...
						money1 = idMap3_2[threadId][reachBegin3[threadId][index5][0]][j].money;
						money2 = elem[j].money;
						pre_money1 = (float)money3 / money1;
						pre_money2 = (float)money4 / money3;
						pre_money3 = (float)money5 / money4;
						pre_money4 = (float)money2 / money5;
						//if(begin==5012) cout<<"h ";     这个点压根没进来
						if (pre_money1 > 3 || pre_money1 < 0.2 || pre_money2>3 || pre_money2 < 0.2)continue;
						if (pre_money3 > 3 || pre_money3 < 0.2 || pre_money4>3 || pre_money4 < 0.2)continue;
						if (visit[next1] == 1 || visit[next2] == 1) continue;
						for (int i = 0; i < 4; i++) {
				...
				for (uint32_t n = 0; n < outDgr[index5]; n++) {
					...
					if (reachBegin3[threadId][index6][1] == begin)
					{
						...
						for (int j = 1; j <= elem[0].sub; ++j)
						{
							...
							money1 = idMap3_2[threadId][reachBegin3[threadId][index6][0]][j].money;
							money2 = elem[j].money;
							pre_money1 = (float)money3 / money1;
							pre_money2 = (float)money4 / money3;
							pre_money3 = (float)money5 / money4;
							pre_money4 = (float)money6 / money5;
							pre_money5 = (float)money2 / money6;
							if (pre_money1 > 3 || pre_money1 < 0.2 || pre_money2>3 || pre_money2 < 0.2)continue;
							if (pre_money3 > 3 || pre_money3 < 0.2 || pre_money4>3 || pre_money4 < 0.2)continue;
							if (pre_money5 > 3 || pre_money5 < 0.2) continue;
							if (visit[next1] == 1 || visit[next2] == 1) continue;
							for (int i = 0; i < 5; i++) {
...
int main()
{
	...
	string testFile = "test_data_ma.txt";
