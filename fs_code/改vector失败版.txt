...
vector<vector<vector<unsigned int>>> idMoney;
vector<vector<vector<subNode>>> idMap;
vector<vector<vector<vector<subNode>>>> idMap3;
...
void searchEdge::init() {
	...
	idMap.resize(threadCount);
	idMap3.resize(threadCount);
	idMoney.resize(threadCount);
...
void searchEdge::remSin()
{
	...
	unsigned int maxDgr = 0;
	for (int i = 1; i <= nodeLen; i++)
	{
		auto elem = i;      //只对起始点判断出度入度
		auto curInDgr = inDgr[elem];
		maxDgr = maxDgr > curInDgr ? maxDgr : curInDgr;
		if (0 == curInDgr)
			que[h++] = elem;
	}
	while (l < h)
	...
	if (maxDgr * 2 < cnt)
	{
		...
		{
			idMoney[i].resize(maxDgr * 2);
			idMap[i].resize(maxDgr * 2);
		}
		if (maxDgr * 3 < cnt)
		{
			for (int i = 0; i < threadCount; ++i)
			{
				idMap3[i].resize(maxDgr * 3);
			}
		}
		else
		{
...
void searchEdge::storeGra3(unsigned int begin, int threadId) {
	unsigned int subLen = 0, subLen3 = 0, pointLen = 0;
	unsigned int ind = 0;
	...
	for (int i = 0; i < inDgr[begin]; i++) {
		...
		for (int k = 0; k < inDgr[gi]; ++k)
		{
			...
			if (reachBegin2[threadId][gk][1] != begin)
			{
				...
				idMoney[threadId][subLen].emplace_back(0);
				subNode elem;
				elem.sub = 0;
				idMap[threadId][subLen++].emplace_back(elem); //数量
				idPoint2[threadId][pointLen++] = gk; //子节点
			}
			ind = reachBegin2[threadId][gk][0];
			subNode subElem;
			subElem.sub = gi;
			subElem.money = pre_money2;
			idMap[threadId][ind].emplace_back(subElem);
			...
			idMoney[threadId][ind].emplace_back(pre_money);
		}
	}
